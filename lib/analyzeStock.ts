...
export function pearsonCorr(a: DailyReturn[], b: DailyReturn[]): number | null {
 ...
 * `onProgress` is called at each stage (used by the SSE route to stream events).
 * Returns the complete QuantAnalysis object.
 */
export async function analyzeStock(
  ...
  const { polygonKey, fmpKey, claudeKey } = keys;
  const send = (stage: string, message: string) => onProgress?.(stage, message);

  // ── Stage 1: Fetch all data ─────────────────────────────────────────────────
  send("fetching", "Fetching Stock Data...");

  const to = dateOffset(0);
  const from730 = dateOffset(730);
  const from2y = dateOffset(504);
  ...
  if (claudeKey) {
    send("researching", "Claude Researching...");
    ...
    try {
      ...
      send("selecting", "Selecting Best Formula...");

      claudeAnalysis = { ...partial, aiAdjustedScore: 0 };
    } catch (err) {
  ...
  // ── Stage 4: Reporting ───────────────────────────────────────────────────────
  send("reporting", "Generating Report...");
